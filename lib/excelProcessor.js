...
const processExcelFile = async (excelFilePath, camposPoblacion, outputDirectory) => {
  try {
    if (!fs.existsSync(excelFilePath)) {
      throw new Error(`El archivo en la ruta ${excelFilePath} no existe.`);
    }
    ...
    for await (const worksheet of workbookOriginal) {
      for await (const row of worksheet) {
        ...
        if (!localidadesData.some(l => l.LOC === localidad && l.MUN === municipio)) {
          localidadesData.push({ LOC: localidad, NOM_LOC: nom_localidad, MUN: municipio, ENTIDAD: entidad });
        }
    ...
    localidadesSheet.columns = [
      ...
      { header: 'MUN', key: 'MUN', width: 15 },
      { header: 'ENTIDAD', key: 'ENTIDAD', width: 15 } // Nueva columna para ENTIDAD
    ];
    ...
    const outputFilePath = path.join(process.cwd(), outputDirectory || 'nuevo_registro_tablas.xlsx');
